...
import { FileText, Download, Copy, ExternalLink, Loader2, RefreshCw } from 'lucide-react'
import { contractTemplates, ContractTemplateData } from '@/lib/contract-template-registry'
import { toast } from 'sonner'
import { updateContractTemplateAction } from '@/app/dashboard/contracts/actions'
...
export function ContractExportModal({ contractId, initialTemplateId, templateData }: ContractExportModalProps) {
  const [open, setOpen] = useState(false)
  const [selectedTemplateId, setSelectedTemplateId] = useState(initialTemplateId || contractTemplates[0]?.id || 'modern-business')
  ...
  async function handleExportPdf() {
    ...
    try {
      const res = await fetch(`/api/contracts/${contractId}/export?template=${selectedTemplateId}`)
  ...
  async function handleExportDocx() {
    ...
    try {
      const res = await fetch(`/api/contracts/${contractId}/export-docx?template=${selectedTemplateId}`)
  ...
  function handleTemplateSelect(tId: string) {
    ...
    startTransition(async () => {
      try {
        await updateContractTemplateAction(contractId, tId)
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-[1200px] w-[90vw] h-[90vh] flex flex-col p-0 overflow-hidden">
        <DialogHeader className="p-4 border-b shrink-0 flex flex-row items-center justify-between">
          <div>
            <DialogTitle className="text-xl">Contract Export</DialogTitle>
            <p className="text-sm text-muted-foreground">Preview and export your contract.</p>
          </div>
